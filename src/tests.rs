...
mod helper {
  ...
  pub fn with_sheet_1<F>(run_test: F) where F: Fn(Sheet, [Coord; 1]) -> () {
      let sheet = Sheet::new(1, 1);
      println!("Testing {:?}", [A1.to_string()]);
      run_test(sheet, [A1]);
  }

  pub fn with_sheet_2<F>(run_test: F) where F: Fn(Sheet, [Coord; 2]) -> () {
    let c = [A1, B1];
    let perms = &[[0, 1], [1, 0]];
    for &[x, y] in perms {
  ...
  pub fn with_sheet_3<F>(run_test: F) where F: Fn(Sheet, [Coord; 3]) -> () {
    let c = [A1, B1, C1];
